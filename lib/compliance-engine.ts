...
export function evaluateLicense(input: LicenseInput): LicenseEvaluation {
  ...
  const eligible = (cert: EngineCertificate) => {
    const types = rule.acceptedCreditTypes?.length ? rule.acceptedCreditTypes : ["AMA_PRA_1", "AOA_1_A"];
    if (cert.creditType && types.includes(cert.creditType)) return true;
  ...
  const topicHours = (cert: EngineCertificate, topic: string): number => {
    ...
    if (cert.manuallyVerified && allocations && typeof allocations === "object" && Object.keys(allocations).length) {
      try {
        const split = validateTopicAllocations(allocations, cert.creditHours ?? 0);
        if (Object.keys(split).some((t) => !allowed.includes(t as EngineRequirement["topic"]))) return 0;
        return Math.min(cert.creditHours ?? 0, split[topic] ?? 0);
      } catch { return 0; }
    }
    // Without a recorded split the same hours can satisfy only one topic.
    return allowed[0] === topic ? cert.creditHours ?? 0 : 0;
  };
  ...
  for (const cert of certificates.filter((c) => inCycle(c) || !c.activityDate)) {
  ...
  for (const req of input.requirements.filter((r) => !r.retiredAt)) {
    ...
    const earned = cycleCertificates.reduce((sum, c) => sum + topicHours(c, req.topic), 0);
    ...
    const completedAt = linked?.activityDate ?? completion?.completedAt ?? (completion?.completedYear ? new Date(Date.UTC(completion.completedYear, 0, 1)) : null);
    ...
    else if (linkedId && linkedHours < req.hoursRequired) status = "NOT_MET";
