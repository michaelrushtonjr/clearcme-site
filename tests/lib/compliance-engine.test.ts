...
test("NV DO psychiatry-only predicate excludes EM, evaluates psychiatry, and leaves other rules alone", () => {
  const input = licenseInput({ requirements: [requirement({ topic: "CULTURAL_COMPETENCY", description: "Psychiatry cultural competency", cadence: "EVERY_RENEWAL" })], practice: { specialty: "Emergency Medicine" } });
  input.license.licenseType = "DO";
  const result = evaluateLicense(input);
  expect(result.requirements[0]).toMatchObject({ status: "NOT_APPLICABLE", gap: 0, reason: "psychiatry-only requirement; not your specialty" });
  for (const practice of [{ specialty: "PSYCHIATRY" }, { specialty: "Medicine", practiceArea: "Consultation psychiatry" }]) {
    expect(evaluateLicense({ ...input, practice }).requirements[0].status).toBe("NOT_MET");
  }
  expect(evaluateLicense({ ...input, license: { ...input.license, state: "CA" } }).requirements[0].status).toBe("NOT_MET");
  expect(evaluateLicense({ ...input, license: { ...input.license, licenseType: "MD" } }).requirements[0].status).toBe("NOT_MET");
  expect(evaluateLicense({ ...input, requirements: [requirement({ topic: "CULTURAL_COMPETENCY", description: "All physicians" })] }).requirements[0].status).toBe("NOT_MET");
  expect(evaluateLicense({ ...input, requirements: [requirement({ topic: "CULTURAL_COMPETENCY", notes: "psychiatrists only" })] }).requirements[0].status).toBe("NOT_APPLICABLE");
});
