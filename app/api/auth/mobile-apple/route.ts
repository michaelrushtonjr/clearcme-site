...
import { exchangeAppleAuthorizationCode } from "@/lib/apple-tokens";
import { prisma } from "@/lib/prisma";
const APPLE_JWKS = createRemoteJWKSet(new URL("https://appleid.apple.com/auth/keys"));
export async function OPTIONS() { return new NextResponse(null, { status: 204, headers: MOBILE_CORS }); }
export async function POST(req: NextRequest) {
  try {
    const { identityToken, fullName, authorizationCode } = await req.json();
    ...
    // Keep a revocable token so account deletion can also revoke Sign in with
    // Apple. Best-effort: sign-in never depends on it.
    if (typeof authorizationCode === "string" && authorizationCode) {
      try {
        ...
        if (refreshToken) await prisma.account.updateMany({ where: { provider: "apple", providerAccountId: payload.sub }, data: { refresh_token: refreshToken } });
      } catch { console.error("Apple refresh-token capture failed; sign-in continues"); }
    }
