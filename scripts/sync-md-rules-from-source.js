...
async function upsertState(state, mdRequirement) {
  const cycleYears = mdRequirement.cycleYears ?? 2;
  const ruleData = {
    state,
    licenseType: 'MD',
    totalHours: mdRequirement.totalHours ?? 0,
    renewalCycle: cycleYears * 12,
    notes: `${mdRequirement.totalHoursLabel}; ${mdRequirement.cycleLabel}`,
  };
  const mandatoryRequirements = mdRequirement.mandatoryTopics.map(toMandatoryRequirement);
  ...
  const existing = await prisma.complianceRule.findUnique({
    where: { state_licenseType: { state, licenseType: 'MD' } },
    select: { id: true },
  });
  const rule = existing
...
async function main() {
  ...
  const results = [];
  for (const state of states) results.push(await upsertState(state, requirements[state].MD));
  console.log(JSON.stringify({ dryRun: DRY_RUN, synced: results }, null, 2));
}
